...
double GaussianFunction3D(double x, double y, double z, double sigma = SIGMA) {
	return GaussianFunction(x, sigma) * GaussianFunction(y, sigma), GaussianFunction(z, sigma);
}
...
bool BackgroundCheck(Vec3b& p1, int flag) {
	/*
	Magenta: (255, 0, 255)
	Gray: (127, 127, 127)
	*/
...
Mat JointBilateralFilteringWithNormalization(Mat src, Mat target, int kernelSize = 9, double sigma = -1, double colorSigma = -1) {
	...
	for (int i = 0; i < res.rows; ++i) {
		for (int j = 0; j < res.cols; ++j) {
			Vec3b tmpVec = src.at<Vec3b>(i, j);
			//if (!BackgroundCheck(tmpVec, MAGENTA)) {
			//	res.at<Vec3f>(i, j) = { 1, 0, 1 };
			//	continue;
			//}
			auto origin = srcNorm.at<Vec3f>(i, j);
			for (int c = 0; c < 3; ++c) {
				double sum = 0.0;
				double sum2 = 0.0;
				int cnt = 0;
				for (int k = 0; k < kernelSize; ++k) {
					for (int l = 0; l < kernelSize; ++l) {
						...
						if (0 <= nx && nx < res.rows && 0 <= ny && ny < res.cols) {
							auto np = srcNorm.at<Vec3f>(nx, ny);
							Vec3b tmpVec2 = src.at<Vec3b>(nx, ny);
							//if (!BackgroundCheck(tmpVec2, MAGENTA)) continue;
							sum2 += filter[k][l] * GaussianFunction3D(origin[0] - np[0], origin[1] - np[1], origin[2] - np[2], colorSigma);
							sum += filter[k][l] * targetNorm.at<Vec3f>(nx, ny)[c] * GaussianFunction3D(origin[0] - np[0], origin[1] - np[1], origin[2] - np[2], colorSigma);
						}
					}
				}
				...
				for (int k = 0; k < kernelSize; ++k) {
					for (int l = 0; l < kernelSize; ++l) {
						...
						if (0 <= nx && nx < res.rows && 0 <= ny && ny < res.cols) {
							...
							if (BackgroundCheck(src.at<Vec3b>(i, j), 0)) continue;
							sum += filter[k][l] * target.at<Vec3b>(nx, ny)[c] * GaussianFunction(origin - np, colorSigma);
							sum2 += filter[k][l] * GaussianFunction(origin - np, colorSigma);
						}
					}
